...
pub mod mymoneydapp {
    ...
    pub fn proxy_transfer(ctx: Context<ProxyTransfer>, amount: u64) -> ProgramResult {
        anchor_spl::token::transfer(ctx.accounts.into(), amount)
    }

    pub fn proxy_mint(ctx: Context<ProxyMintTo>, amount: u64) -> ProgramResult {
    ...
    pub fn proxy_set_authority(
        ...
        anchor_spl::token::set_authority(ctx.accounts.into(), authority_type.into(), new_authority)
    }
}
...
impl<'a, 'b, 'c, 'info> From<&mut ProxyTransfer<'info>>
    for CpiContext<'a, 'b, 'c, 'info, Transfer<'info>> {
        fn from(
            accounts: &mut ProxyTransfer<'info>
